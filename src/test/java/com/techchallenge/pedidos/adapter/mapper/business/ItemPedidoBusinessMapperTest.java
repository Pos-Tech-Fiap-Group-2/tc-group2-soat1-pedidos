...
public class ItemPedidoBusinessMapperTest {

	@InjectMocks
	private ItemPedidoBusinessMapper api;
	...
	public void toModel() {
		...
		ItemPedidoEntity entity = createItemPedidoEntity(itemPedido);
		
		when(mapper.map(entity, ItemPedido.class)).thenReturn(itemPedido);
		assertEquals(api.toModel(entity), itemPedido);
	}
	...
	public void toCollectionModel() {
		...
		List<ItemPedidoEntity> entities = createItemPedidoEntities(itens);
		
		when(mapper.map(itens.stream().findFirst().get(), ItemPedido.class)).thenReturn(itens.stream().findFirst().get());
