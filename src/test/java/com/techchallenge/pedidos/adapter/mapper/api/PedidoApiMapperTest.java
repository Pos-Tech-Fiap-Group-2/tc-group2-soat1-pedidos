...
public class PedidoApiMapperTest {

	@InjectMocks
	private PedidoApiMapper api;
	...
	private List<ItemPedidoInput> createItemPedidoInputs(List<ItemPedido> itens) {
	...
	private PedidoInput createPedidoInput(Pedido pedido) {
		...
		input.setItens(createItemPedidoInputs(pedido.getItens()));
    ...
    public void toDomainObject() {
		...
		PedidoInput input = createPedidoInput(pedido);

        when(mapper.map(input, Pedido.class)).thenReturn(pedido);
        assertEquals(api.toDomainObject(input), pedido);
    }
    ...
    public void toModel() {
		...
		when(mapper.map(pedido, PedidoModel.class)).thenReturn(model);
		assertEquals(api.toModel(pedido), model);
    }
    ...
    public void toCollectionModel() {
		...
		when(mapper.map(pedido, PedidoModel.class)).thenReturn(model);
		assertTrue(api.toCollectionModel(pedidos).size() > 0);
    }

	@Test
    public void toCollectionModelOrderByStatus() {
    	List<Pedido> pedidos = new ArrayList<>();
		Cliente cliente = createCliente(1L, 12345678901L, "cliente.teste@teste.com.br", "Cliente Teste");
		Pedido pedido = createPedido(1L, cliente);
		PedidoModel model = createPedidoModel(pedido);
		...
		when(mapper.map(pedido, PedidoModel.class)).thenReturn(model);
		assertTrue(api.toCollectionModelOrderByStatus(pedidos).size() > 0);
    }
	
	@Test
    public void toCollectionModelOrderByStatus_Finalizado() {
    	List<Pedido> pedidos = new ArrayList<>();
		Cliente cliente = createCliente(1L, 12345678901L, "cliente.teste@teste.com.br", "Cliente Teste");
		Pedido pedido = createPedido(1L, cliente, StatusPedido.FINALIZADO);
		PedidoModel model = createPedidoModel(pedido);
