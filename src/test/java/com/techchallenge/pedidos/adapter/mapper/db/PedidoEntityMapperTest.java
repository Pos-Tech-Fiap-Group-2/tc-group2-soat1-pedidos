...
public class PedidoEntityMapperTest {
	...
	@InjectMocks
	private PedidoEntityMapper api;
	...
	public void toModel() {
    	...
    	doNothing().when(mapper).map(pedido, entity);
    	assertDoesNotThrow(() -> api.toModel(pedido, entity));
	}
    
    @Test
	public void toModel_Pedido() {
    	Cliente cliente = createCliente(1L, 12345678901L, "cliente.teste@teste.com.br", "Cliente Teste");
		Pedido pedido = createPedido(1L, cliente);
    	PedidoEntity entity = createPedidoEntity(pedido);
    	PedidoEntity expected = createPedidoEntity(pedido);
    	
    	when(mapper.map(entity, PedidoEntity.class)).thenReturn(expected);
    	assertEquals(api.toModel(entity), expected);
	}
	
    @Test
	public void toModel_Entity() {
    	Cliente cliente = createCliente(1L, 12345678901L, "cliente.teste@teste.com.br", "Cliente Teste");
		Pedido pedido = createPedido(1L, cliente);
    	PedidoEntity entity = createPedidoEntity(pedido);
    	
    	when(mapper.map(pedido, PedidoEntity.class)).thenReturn(entity);
    	assertEquals(api.toModel(pedido), entity);
	}
